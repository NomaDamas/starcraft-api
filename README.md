...
Use `--replace-running` with `--launch` only for controlled direct-launch tests where the current StarCraft game process must be replaced with a new single instance. This is useful when changing `STARCRAFT_API_EXTRA_ARGS`, which appends quoted arguments after the default Remastered launch flags, for example `STARCRAFT_API_EXTRA_ARGS='playReplay "Maps/Replays/test.rep"'`.
