...
The Remastered backend advertises the built-in BWAPI facade surface when no manifest is supplied: 385 abstract API methods. The 44 unit commands and 28 game actions are known to the portable command queue/encoder, but the backend does not advertise them as implemented command surface until a validated adapter proves `issue-commands` and `draw-overlays` with an active command receiver, runtime command queue sink, and proof-backed command queue bindings. This keeps deterministic dry-run command serialization separate from in-game command delivery. Actual state extraction, command delivery, overlay rendering, event dispatch, replay analysis, multiplayer synchronization, and AI module loading remain gated by versioned runtime bindings plus live adapter proof lines.
