...
`starcraft-runtime-adapter-proof --prove-read-map-data` scans live memory for installed map names/paths and falls back to fresh replay artifacts when the live string proof is unavailable. Set `STARCRAFT_API_REPLAY_DIR` to override replay discovery. A replay fallback ready file records `proof.read_map_data.source=latest-replay-artifact`, `proof.read_map_data.replay_path`, and `proof.read_map_data.replay_file_size`; this is map-data evidence only and does not satisfy command, overlay, or multiplayer proof requirements.
