...
`starcraft-runtime-adapter-proof --discover-command-queue` scans readable+writable live memory for command-queue-like vector candidates and writes `command_queue.candidates.tsv` plus `proof.command_queue_discovery=*` metadata into the bridge. This is discovery-only evidence: it does not write `proof.issue_commands=passed`, does not resolve `BW::BWDATA::sgdwBytesInCmdQueue`/`TurnBuffer`, and does not make `starcraft-runtime-submit-command` accept commands. Promote a candidate only after a resident adapter proves that encoded BWAPI payloads are consumed by the live SC:R command path and produce in-game behavior.
